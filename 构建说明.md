...
| 变量 | 说明 | 示例 |
|------|------|------|
| `DEEPSEEK_API_KEY` | DeepSeek API 密钥（必填，或通过 keyring 配置） | `sk-...` |
| `DATABASE_URL` | SQLite 库路径 | `sqlite://chat.db` |
| `DEEPSEEK_MODEL_ID` 或 `MODEL_ID` | 模型 ID，默认 `deepseek/deepseek-chat` | `deepseek/deepseek-chat` |
...
A: 设置 `DEEPSEEK_API_KEY`，或按 ai-lib-rust 文档配置 keyring。

**Q: 依赖的 ai-lib-rust 来源**  
A: `Cargo.toml` 中为 `ai-lib-rust = "0.5.1"`，从 **crates.io** 拉取线上版本；无需本地 ai-lib-rust 源码。如需本地调试可临时改为 `{ path = "../ai-lib-rust" }`。
